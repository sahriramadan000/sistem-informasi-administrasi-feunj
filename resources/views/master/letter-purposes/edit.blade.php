...
<div class="max-w-4xl mx-auto">
    ...
    <div class="flex justify-between items-center mb-6">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 flex items-center gap-2">
                ...
                Edit Keperluan Surat
            </h2>
            <p class="mt-1 text-sm text-gray-500">Ubah informasi keperluan yang ada</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow border border-gray-200">
        <div class="p-6">
            <form method="POST" action="{{ route('master.letter-purposes.update', $letterPurpose) }}">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" 
                               id="name" 
                               name="name" 
                               value="{{ old('name', $letterPurpose->name) }}" 
                    ...
                    <div class="space-y-2">
                        ...
                        <textarea id="description" 
                                  ...
                                  placeholder="Masukkan deskripsi keperluan">{{ old('description', $letterPurpose->description) }}</textarea>
                        @error('description')
                            <p class="text-sm text-destructive flex items-center gap-1.5">
                                <i data-lucide="alert-circle" class="w-4 h-4"></i>
                                {{ $message }}
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input type="checkbox" 
                                       ...
                                       name="is_active" 
                                       value="1" 
                                       {{ old('is_active', $letterPurpose->is_active) ? 'checked' : '' }}
                            ...
                            <div class="ml-3">
                                ...
                                <p class="text-sm text-muted-foreground">Centang jika keperluan ini dapat digunakan untuk membuat surat</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="bg-orange-50 border border-blue-200 rounded-lg p-4">
                            ...
                            <ul class="list-disc list-inside text-sm text-brand-darker space-y-1">
                                <li>Keperluan akan ditampilkan pada jenis surat yang memerlukan keperluan</li>
                                <li>Nonaktifkan keperluan jika tidak lagi digunakan</li>
                                <li>Perubahan tidak akan mempengaruhi surat yang sudah dibuat sebelumnya</li>
                            </ul>
                        </div>
                    </div>
                </div>
            ...
            @if(auth()->user()->isAdmin())
                <form method="POST" 
                      action="{{ route('master.letter-purposes.destroy', $letterPurpose) }}" 
                    ...
                    <div class="flex items-center justify-between bg-red-50 border border-red-200 rounded-lg p-4">
                        <div>
                            <h6 class="font-semibold text-red-900 mb-1">Hapus Keperluan Surat</h6>
                            <p class="text-sm text-red-700">Tindakan ini tidak dapat dibatalkan. Pastikan tidak ada surat yang menggunakan keperluan ini.</p>
                        </div>
